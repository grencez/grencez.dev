...
- `alp-3-host` as an Alpine LXC that acts as a simple IPv6 router.
- `eth0` as `alp-3-host`'s WAN interface that obtains IPv6 addresses via DHCPv6.
  - `vmbr0` on the Proxmox host. This should already exist.
- `eth1` as `alp-3-host`'s LAN interface that provides IPv6 addresses via SLAAC.
  - `vmbr3` on the Proxmox host. We'll create this too.
- `alp-4-host` as an Alpine LXC whose traffic will go through `alp-3-host`.
  - We'll give it a simple firewall in Proxmox that blocks inbound traffic.

## Setup
### Router LXC
Let's set up `alp-3-host` along with its network bridge interface `vmbr3`.
...
- Click: A datacenter node (i.e., the Proxmox host) -> Network -> Create -> Linux Bridge
  - Name: `vmbr3`
- Click: `alp-3-host` -> Network -> Add
  - Name: `eth1`
  - Bridge: `vmbr3`
  - Firewall: No
  - IPv4 and IPv6: Static

Now boot up the new `alp-3-host` LXC and login as root.
...
Verify that `alp-3-host` can reach the internet over IPv6:
```shell
nslookup ip6only.me
```
...
Verify that `alp-3-host` can still reach the internet over IPv6:
```shell
nslookup ip6only.me
ping -6 -c 3 grencez.dev
```

Verify that `alp-3-host` has 2 IPv6 addresses for `eth0` and another 2 for `eth1`:
```shell
...
### SLAAC Router
Configure `radvd` to advertise that `alp-3-host` can act as a router for SLAAC clients on `eth1`:
```shell
apk add radvd
...
## Test
### Client LXC
In order to test, let's create the client LXC named `alp-4-host`.
...
- Click: `alp-4-host` -> Firewall -> Options
  - Firewall: Yes
  - NDP: Yes
  - log_level_in: info
  - log_level_out: nolog
...
### Client Connectivity
Boot up the new `alp-4-host` LXC and hope for the best!
...
On `alp-4-host`, grab one of the addresses returned by `ip address` and try pinging it from `alp-3-host`.
The ping command should not show any responses; 100% packet loss.

You should also be able to see these DROPped ping attempts in the Proxmox UI at `alp-4-host` -> Firewall -> Log.
...
- **DNS server.** The two LXC hosts we configured in this article cannot find each other by name. It would be best if `alp-3-host` could also act as a DNS server.
